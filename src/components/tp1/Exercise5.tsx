...
export default function Exercise5() {
  ...
  const deltaPrimeApprox = (t: number, w: number = 0.05) => {
    // Une approximation simple de delta'(t) : une impulsion positive suivie d'une négative
    ...
    <div className="space-y-6 pb-20">
      <header>
        <h2 className="text-2xl font-bold">
          Exercice 5 — Transformée de Fourier & Théorème de Dérivée
        </h2>
        <p className="text-sm text-muted-foreground">
          Calculer la TF du signal en déduisant la Transformée de la dérivée
        </p>
      </header>
      ...
      <GlassCard className="p-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          ...
          <br />
          <br />
          On trouve donc en fonction des fenêtres Rect :
      ...
      <GlassCard className="p-6 mt-6 border-l-4 border-l-emerald-500">
        <h3 className="text-lg font-semibold mb-3 text-emerald-400">
          4. Déduction par Théorème de la Dérivée (TF)
        </h3>
        <div className="text-sm text-foreground/80 space-y-3 font-mono">
          ...
          <hr className="border-white/10" />
          <p className="text-emerald-300 bg-emerald-500/10 p-4 rounded text-center border border-emerald-500/20">
            Y(f) = [ j(4πf - 2sin(2πf)) ] / [ -4π²f² ]
            <br /><br />
            <strong>Y(f) = -j · [ 2πf - sin(2πf) ] / [ 2π²f² ]</strong>
          </p>
        </div>
      </GlassCard>
    </div>
